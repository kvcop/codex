...
//!    separators, or fallback to record-list format when the minimum cannot
//!    fit or the wrapped table would become hard to scan.
...
//! preserved last.  When even 3-char-wide columns cannot fit, or when a dense
//! table would wrap into very tall rows, the table falls back to a record-list
//! layout.
...
///
/// `table_lines` are either prewrapped aligned rows or record-list
