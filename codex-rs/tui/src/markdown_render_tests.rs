...
#[test]
fn table_falls_back_to_key_value_records_when_wrapped_rows_are_too_tall() {
...
| Experiment | product | backend | EU | Growth | 12000 | 2026-05-20 | 14:00 | Chasm reduction | Watching | The experiment is in final preparation and depends on baseline stability from last week. Check that thresholds do not drift because of seasonality and outside traffic before continuing. |\n";
    let text = crate::markdown_render::render_markdown_text_with_width(md, Some(/*width*/ 88));
    let lines = plain_lines(&text);

    assert!(lines.iter().any(|line| line.contains("Category")));
    assert!(lines.iter().any(|line| line.contains("Experiment")));
    assert!(
        lines
            .iter()
            .any(|line| line.contains("The experiment is in final preparation"))
    );
    assert!(!lines.iter().any(|line| line.contains('┌')));
    assert!(!lines.iter().any(|line| line.contains('━')));
}
