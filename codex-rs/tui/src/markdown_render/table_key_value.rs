...
const MAX_GRID_ROW_HEIGHT: usize = 6;
const NARROW_COLUMN_WIDTH: usize = 4;
const NARROW_HEADER_MIN_COLUMNS: usize = 6;
...
pub(super) fn should_render_records(
    ...
    if rows
        .iter()
        .any(|row| grid_row_height(row, column_widths) > MAX_GRID_ROW_HEIGHT)
    {
        return true;
    }
    ...
    let narrow_columns = column_widths
        .iter()
        .filter(|width| **width <= NARROW_COLUMN_WIDTH)
        .count();
    let header_is_cramped = metrics
        .iter()
        .zip(column_widths)
        .any(|(metrics, width)| metrics.header_token_width > *width);
    if column_widths.len() >= NARROW_HEADER_MIN_COLUMNS
        && header_is_cramped
...
fn grid_row_height(row: &[TableCell], column_widths: &[usize]) -> usize {
    row.iter()
        .zip(column_widths)
        .map(|(cell, width)| wrap_cell(cell, *width).len())
        .max()
        .unwrap_or(1)
}
