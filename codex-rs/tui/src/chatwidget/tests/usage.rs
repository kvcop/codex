...
async fn rate_limit_reset_popup_states_snapshot() {
    ...
    assert!(chat.finish_rate_limit_reset_credits_refresh(
        loading_request_id,
        vec![snapshot_with_reset_windows(
...
#[tokio::test]
async fn usage_limit_reset_confirmation_requires_weekly_safety_data() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    set_chatgpt_auth(&mut chat);
...
#[tokio::test]
async fn rate_limit_reset_confirmation_blocks_immediate_default_enter() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    set_chatgpt_auth(&mut chat);
    ...
    assert_no_reset_usage_confirmed(&mut rx);
    chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
...
async fn rate_limit_reset_confirmation_can_use_reset() {
    ...
    assert_matches!(
        next_reset_usage_confirmed(&mut rx),
        AppEvent::ResetUsageConfirmed { attempt }
            if Uuid::parse_str(&attempt.idempotency_key).is_ok()
                && attempt.available_count == 1
                && attempt.remaining_percent == 10
    );
}
...
async fn rate_limit_reset_load_result_updates_popup_beneath_overlay() {
    ...
    assert!(popup.contains("Usage limit reset is available."));
    assert!(popup.contains("/reset-usage"));
}
...
fn assert_no_reset_usage_confirmed(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
    for event in std::iter::from_fn(|| rx.try_recv().ok()) {
        assert!(
            !matches!(event, AppEvent::ResetUsageConfirmed { .. }),
            "unexpected reset usage confirmation: {event:?}"
        );
    }
}

fn next_reset_usage_confirmed(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) -> AppEvent {
    loop {
        match rx.try_recv() {
            Ok(event @ AppEvent::ResetUsageConfirmed { .. }) => return event,
            Ok(_) => continue,
            Err(error) => panic!("expected reset usage confirmation event, got {error:?}"),
        }
    }
}
