...
impl ChatWidget {
    ...
    pub(crate) fn finish_usage_menu_rate_limit_refresh(
        ...
        if let Ok(response) = result {
            self.on_rate_limit_reset_credits(Some(response));
        }
        ...
        let params = match result {
            Ok(response) => {
                ...
                if available_count > 0 {
                    return if self
                        .bottom_pane
                        .dismiss_active_view_if_id(RATE_LIMIT_RESET_VIEW_ID)
                    {
                        self.open_reset_usage_prompt();
                        true
                    } else {
                        self.bottom_pane.replace_selection_view_if_present(
                            RATE_LIMIT_RESET_VIEW_ID,
                            Self::rate_limit_reset_message_params(
                                "Usage limit reset is available. Close overlays and run /reset-usage to confirm.",
                            ),
                        )
                    };
                }
    ...
    pub(crate) fn finish_rate_limit_reset_consume(
        ...
        match result {
            ...
            {
                self.on_rate_limit_reset_credits(None);
            ...
            Ok(response) => {
                ...
                let message = match response.outcome {
                    ...
                    ConsumeAccountRateLimitResetCreditOutcome::NoCredit => {
                        self.on_rate_limit_reset_credits(Some(RateLimitResetCreditsSummary {
                            available_count: 0,
                        }));
